...
tasks.register("generateVersionInfo") {
    group = "build"
    description = "Generates VersionInfo.kt with the project version for use in main source set"

    // Capture version in configuration phase for configuration cache compatibility
    val serverVersion = centralVersion
    inputs.property("serverVersion", serverVersion)
    val outDir = layout.buildDirectory.dir("generated/version/main/kotlin")
    outputs.dir(outDir)

    doLast {
        val packagePath = "eu/torvian/chatbot/server"
        ...
        versionInfoFile.writeText(
            """
            package eu.torvian.chatbot.server

            /**
             * Version information for the Chatbot server.
             * This file is auto-generated by the generateVersionInfo task.
             */
            object VersionInfo {
                const val VERSION = "$serverVersion"
            }
            """.trimIndent()
        )

        println("Generated VersionInfo.kt with version: $serverVersion")
    }
}

// Register the generated source directory for main using task provider (configuration-cache friendly)
