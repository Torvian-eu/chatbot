...
/**
 * Route composable for the Server Built-In Tools settings category.
 *
 ...
 * [ServerBuiltInToolsTab].
 *
 * @param authState Authentication context for the active session.
 * @param viewModel Server Built-In Tools ViewModel resolved from Koin.
 ...
 *   Server Built-In Tools page in the breadcrumb trail.
 */
@Composable
fun ServerBuiltInToolsTabRoute(
    authState: AuthState.Authenticated,
    modifier: Modifier = Modifier,
    viewModel: ServerBuiltInToolsViewModel = koinViewModel(),
    ...
    // Update breadcrumbs.
    LaunchedEffect(Unit) {
        onBreadcrumbsChanged(listOf("Settings", SettingsCategory.ServerBuiltInTools.displayLabel))
    }
    ...
    val actions = object : ServerBuiltInToolsTabActions {
        override fun onLoadTools() = viewModel.loadToolsIfNeeded()
        override fun onToggleToolEnabled(tool: ServerBuiltInToolDefinition) =
            viewModel.toggleToolEnabled(tool)
        override fun onUpdateTool(tool: ServerBuiltInToolDefinition) = viewModel.updateTool(tool)
