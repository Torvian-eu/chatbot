...
/**
 * Settings specifically for [LLMModelType.RESPONSES] models. These models are accessed through
 * OpenAI's Responses API and support native reasoning tokens that can be replayed across turns.
 * The parameter surface mirrors [ChatModelSettings] (translated to Responses-equivalent wire
 * parameters such as `instructions` and `max_output_tokens`) while adding Responses-specific knobs
 * for reasoning effort, stateful storage, and reasoning replay.
 *
 * @property id Unique identifier for the settings profile (Database PK).
 * @property modelId Foreign key to the associated [LLMModel].
 * @property name The display name of the settings profile (e.g., "Default", "Creative").
 ...
 *                          mean more random outputs, lower values mean more deterministic outputs.
 *                          Typically between 0.0 and 2.0.
 * @property maxOutputTokens Maximum number of tokens to generate in the response, mapped to the Responses
 *                              `max_output_tokens` field.
 * @property topP Top P sampling (nucleus sampling) for text generation. Controls diversity by
 *                   considering only the most probable tokens whose cumulative probability exceeds `topP`.
 *                   Typically between 0.0 and 1.0.
 * @property stopSequences A list of strings that, if generated by the model, will cause it to stop
 *                             generating further tokens immediately. Mapped to the Responses `stop` field.
 * @property stream Whether to stream the response back as it's generated.
 * @property reasoningEffort The effort level used for reasoning (`low`, `medium`, or `high`). Mapped to the
 *                              Responses `reasoning.effort` field. Null lets the provider use its default.
 * @property store Whether to store the response server-side so it can be referenced via `previous_response_id`
 *                    in subsequent turns. Enabling this persists reasoning and tool context between turns.
 * @property replayReasoning Whether to replay previous reasoning items back into the next request's `input`.
 *                              This preserves reasoning context even when [store] is disabled.
 * @property customParams Arbitrary model-specific or provider-specific parameters not covered
 *                        by strongly-typed fields, stored as a [JsonObject].
 */
@Serializable
@SerialName("responses")
data class ResponsesModelSettings(
    override val id: Long,
    override val modelId: Long,
    override val name: String,
