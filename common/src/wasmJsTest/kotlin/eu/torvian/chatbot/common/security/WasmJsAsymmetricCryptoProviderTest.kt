...
/**
 * Test suite for WasmJsAsymmetricCryptoProvider.
 *
 * This tests the WASM/JS Ed25519 signing and verification implementation
 * that uses the browser Web Crypto Subtle API.
 */
@OptIn(ExperimentalEncodingApi::class)
class WasmJsAsymmetricCryptoProviderTest {

    private val cryptoProvider = WasmJsAsymmetricCryptoProvider()
    ...
    fun `generateKeyPair should return a valid key pair`() = runTest {
        ...
        // Ed25519 public key is 32 bytes (raw format)
        assertEquals(32, keyPair.publicKey.size, "Public key should be 32 bytes for Ed25519")

        // Private key in pkcs8 format is larger (typically 48+ bytes)
        assertTrue(keyPair.privateKey.size > 32, "Private key should be larger than 32 bytes (pkcs8 format)")
    }
    ...
    fun `sign should return a valid Base64 encoded signature`() = runTest {
        ...
        val decoded = Base64.decode(signature)
        assertTrue(decoded.isNotEmpty(), "Signature should decode to non-empty bytes")
    }

    @Test
    fun `sign should return different signatures for same data due to random operations`() = runTest {
        // Arrange
        ...
        // Note: Ed25519 signatures are deterministic, so they should be the same
    ...
    fun `verify should return false for invalid signature`() = runTest {
        ...
        val decoded = Base64.decode(validSignature)
        val tampered = decoded.copyOf()
        if (tampered.isNotEmpty()) {
            tampered[0] = tampered[0].inc() // Modify first byte
        }
        val tamperedSignature = Base64.encode(tampered)
    ...
    fun `sign should return error for invalid private key`() = runTest {
        // Arrange
        val data = "Test message"
        val invalidPrivateKey = ByteArray(16) { 0 } // Too short for Ed25519 pkcs8
    ...
    fun `verify should return error for invalid public key`() = runTest {
        // Arrange
        val data = "Test message"
        val invalidPublicKey = ByteArray(16) { 0 } // Too short for Ed25519 raw (should be 32 bytes)
