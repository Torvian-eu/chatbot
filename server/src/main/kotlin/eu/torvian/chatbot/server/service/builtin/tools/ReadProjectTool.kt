...
/**
 * `read_project` server built-in tool.
 *
 * Returns the ownership-checked [ProjectDto] for one project id. Not-found and not-accessible
 * collapse into a single message so the tool never leaks the existence of another user's project
 * (id-enumeration guard).
 *
 * @property projectService User-scoped project service used for the ownership-checked lookup.
...
class ReadProjectTool(
    ...
    override val name: String = ServerBuiltInToolCatalog.READ_PROJECT_NAME
    ...
    override suspend fun execute(
        ...
        val project = projectService.getProjectById(context.userId, projectId!!)
            .mapLeft {
