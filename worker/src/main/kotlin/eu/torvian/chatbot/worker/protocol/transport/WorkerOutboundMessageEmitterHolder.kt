...
/**
 * Mutable bridge that keeps [WorkerOutboundMessageEmitter] stable for runtime components while
 * allowing transport code to bind and unbind the active socket sender.
 */
class WorkerOutboundMessageEmitterHolder : WorkerOutboundMessageEmitter {
    /**
     * Atomic reference to the currently bound transport emitter, if any.
     */
    private val activeEmitterRef: AtomicReference<WorkerOutboundMessageEmitter?> = AtomicReference(null)
     ...
     */
    fun bind(emitter: WorkerOutboundMessageEmitter) {
    ...
    companion object {
         ...
         */
        private val logger: Logger = LogManager.getLogger(WorkerOutboundMessageEmitterHolder::class.java)
    }
}
