 ...
 */
class McpRuntimeCommandInteractionTest {
    /**
    ...
    fun `test connection emits deterministic dummy result`() = kotlinx.coroutines.test.runTest {
        ...
        assertEquals(DummyMcpRuntimeCommandExecutor.DUMMY_DISCOVERED_TOOL_COUNT, resultData.discoveredToolCount)
        assertEquals(DummyMcpRuntimeCommandExecutor.DUMMY_TEST_CONNECTION_MESSAGE, resultData.message)
    }
    ...
    /**
     * Builds a worker local MCP runtime command interaction configured with the deterministic dummy executor.
     *
    ...
    private fun buildInteraction(
        ...
        executor: McpRuntimeCommandExecutor = DummyMcpRuntimeCommandExecutor()
    ): McpRuntimeCommandInteraction {
     ...
     */
    private class FailingStartExecutor : McpRuntimeCommandExecutor {
        /**
