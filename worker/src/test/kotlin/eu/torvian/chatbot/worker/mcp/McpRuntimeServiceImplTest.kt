...
/**
 * Unit tests for [McpRuntimeServiceImpl].
 */
class McpRuntimeServiceImplTest {
    /**
    ...
    fun `start fails when server config is missing`() = runTest {
        ...
        assertIs<McpRuntimeError.ServerConfigMissing>(error)
    ...
    fun `start is idempotent when already connected`() = runTest {
        val client = FakeWorkerMcpClientService(
            startResult = McpClientStartError.AlreadyConnected(DEFAULT_SERVER_ID).left()
        )
        val service = buildService(clientService = client)
    ...
    fun `stop is idempotent when already stopped`() = runTest {
        val client = FakeWorkerMcpClientService(
            stopResult = McpClientStopError.NotConnected(DEFAULT_SERVER_ID).left()
        )
        val service = buildService(clientService = client)
    ...
    fun `get runtime status maps process and connection metadata`() = runTest {
        ...
        val processManager = FakeMcpProcessManager(
    ...
    private fun buildService(
        configStore: McpServerConfigStore = SingleServerConfigStore(),
        clientService: FakeWorkerMcpClientService = FakeWorkerMcpClientService(),
        processManager: McpProcessManager = FakeMcpProcessManager()
    ): McpRuntimeServiceImpl {
        return McpRuntimeServiceImpl(
            configStore = configStore,
     ...
     */
    private class SingleServerConfigStore : McpServerConfigStore {
        /**
    ...
    private class FakeWorkerMcpClientService(
        private val startResult: Either<McpClientStartError, Unit> = Unit.right(),
        private val stopResult: Either<McpClientStopError, Unit> = Unit.right(),
        private val discoverToolsResult: Either<McpClientDiscoverToolsError, List<Tool>> = emptyList<Tool>().right(),
        ...
        override suspend fun pingClient(serverId: Long): Either<McpClientPingError, Unit> = Unit.right()
        ...
        override fun getConnectionStatus(serverId: Long): McpClientConnectionStatus? {
            ...
            return McpClientConnectionStatus(
                connectedAt = now,
                lastActivityAt = now
            )
        }
     ...
     */
    private class FakeMcpProcessManager(
        private val status: McpProcessStatus = McpProcessStatus(
            serverId = DEFAULT_SERVER_ID,
            state = McpProcessState.STOPPED
        )
    ) : McpProcessManager {
        override suspend fun startServer(config: LocalMCPServerDto): Either<WorkerLocalMcpStartProcessError, McpProcessStatus> =
        ...
        override suspend fun getServerStatus(serverId: Long): McpProcessStatus =
