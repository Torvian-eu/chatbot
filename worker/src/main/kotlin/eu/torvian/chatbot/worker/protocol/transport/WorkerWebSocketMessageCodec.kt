 ...
 */
class WorkerWebSocketMessageCodec(
     ...
     */
    fun decode(text: String): Either<WorkerWebSocketMessageCodecError, WorkerProtocolMessage> {
        return try {
            json.decodeFromString<WorkerProtocolMessage>(text).right()
        } catch (error: SerializationException) {
            WorkerWebSocketMessageCodecError.DecodeFailed(
