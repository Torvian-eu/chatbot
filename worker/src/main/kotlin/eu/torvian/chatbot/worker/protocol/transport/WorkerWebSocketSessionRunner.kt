...
import eu.torvian.chatbot.worker.protocol.handshake.WorkerSessionHelloStartResult
 ...
 */
class WorkerWebSocketSessionRunner(
    private val client: HttpClient,
    private val transportConfig: WorkerWebSocketTransportConfig,
    private val codec: WorkerWebSocketMessageCodec,
    private val outboundEmitterHolder: WorkerOutboundMessageEmitterHolder,
    private val helloStarter: WorkerSessionHelloStarter,
    private val incomingMessageProcessor: WorkerIncomingMessageProcessor
) {
    /**
     ...
     */
    suspend fun run(accessToken: String): WorkerWebSocketSessionResult {
        ...
        return try {
            ...
            WorkerWebSocketSessionResult(stableConnection = true, authRejected = false)
            ...
            WorkerWebSocketSessionResult(stableConnection = false, authRejected = authRejected)
        } catch (error: Exception) {
            logger.warn("Worker WebSocket session failed", error)
            WorkerWebSocketSessionResult(stableConnection = false, authRejected = false)
        } finally {
    ...
    companion object {
         ...
         */
        private val logger: Logger = LogManager.getLogger(WorkerWebSocketSessionRunner::class.java)
    }
}
