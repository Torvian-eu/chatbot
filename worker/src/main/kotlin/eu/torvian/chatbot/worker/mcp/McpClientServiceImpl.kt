...
/**
 * Default [McpClientService] implementation backed by MCP SDK client + stdio transport.
 *
...
class McpClientServiceImpl(
    ...
    companion object {
         ...
         */
        private val logger: Logger = LogManager.getLogger(McpClientServiceImpl::class.java)
    }
     ...
     */
    private val connectionStatusByServerId: ConcurrentHashMap<Long, McpClientConnectionStatus> =
        ConcurrentHashMap()
    ...
    override suspend fun startAndConnect(config: LocalMCPServerDto): Either<McpClientStartError, Unit> {
        ...
        if (processInput == null || processOutput == null) {
            ...
            return McpClientStartError.StreamsUnavailable(
                serverId = serverId,
        ...
        return try {
            ...
            val now = clock.now()
            connectionStatusByServerId[serverId] = McpClientConnectionStatus(
                connectedAt = now,
                lastActivityAt = now
            )
            ...
            processManager.stopServer(serverId)
            McpClientStartError.ConnectionFailed(
                serverId = serverId,
    ...
    private suspend fun stopServerInternal(
        serverId: Long,
        fromTimer: Boolean
    ): Either<McpClientStopError, Unit> {
        logger.info(
        ...
        val client = clientByServerId.remove(serverId)
            ?: run {
                ...
                return McpClientStopError.NotConnected(serverId).left()
            }
        ...
        try {
            ...
            processManager.stopServer(serverId)
            return McpClientStopError.DisconnectFailed(
                serverId = serverId,
                reason = exception.message ?: "Unknown error",
                cause = exception
            ).left()
        ...
        processManager.stopServer(serverId).fold(
            ifLeft = { stopError ->
                if (stopError !is WorkerLocalMcpStopProcessError.ProcessNotRunning) {
                    ...
                    return McpClientStopError.ProcessStopFailed(
                        serverId = serverId,
    ...
    override suspend fun discoverTools(serverId: Long): Either<McpClientDiscoverToolsError, List<Tool>> {
        ...
        val client = clientByServerId[serverId]
            ?: run {
                ...
                return McpClientDiscoverToolsError.NotConnected(serverId).left()
            }
        ...
        return try {
            ...
            McpClientDiscoverToolsError.ListToolsFailed(
                serverId = serverId,
                reason = exception.message ?: "Unknown error",
                cause = exception
            ).left()
    ...
    override suspend fun pingClient(serverId: Long): Either<McpClientPingError, Unit> {
        ...
        val client = clientByServerId[serverId]
            ?: run {
                ...
                return McpClientPingError.NotConnected(serverId).left()
            }
    ...
    override suspend fun callTool(
        ...
        val client = clientByServerId[serverId]
            ?: run {
                ...
                return McpClientCallToolError.NotConnected(serverId = serverId, toolName = toolName).left()
            }
        ...
        return try {
            ...
            McpClientCallToolError.ToolCallFailed(
                serverId = serverId,
                toolName = toolName,
                reason = exception.message ?: "Unknown error",
                cause = exception
    ...
    override suspend fun close() {
        logger.info("Closing McpClientServiceImpl and disconnecting all managed MCP clients")
        ...
        logger.info("McpClientServiceImpl closed")
    }
